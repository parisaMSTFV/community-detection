...
def test_pipeline_writes_required_artifacts(tmp_path: Path) -> None:
    ...
    required = [
        ...
        "reports/temporal_null_distribution.csv",
    ...
    assert metrics["quality_gate"]["status"] in {"pass", "review_required"}
    assert metrics["temporal"]["future_edge_agreement"]["unseen_edge_weight_share"] > 0
